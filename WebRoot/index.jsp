	...
	<%
			...
			if(cookies!=null){
				for (int i = 0; i < cookies.length; i++) {
					...
					//String value = temp.getValue();//值  allowAutoLogin
				...
				<tr>
					<td colspan="3">
						<div align="right" class="myspan" style="background-color: #FAEDs1;">
										<span>${pagemsg}</span>
										<a href="tblFwxx!queryAllPage.action?pageno=1">首页</a>
										<a href="tblFwxx!queryAllPage.action?pageno=${pageno-1}">上一页</a>
										<a href="tblFwxx!queryAllPage.action?pageno=${pageno+1}">下一页</a>
										<a href="tblFwxx!queryAllPage.action?pageno=${pageMax}">尾页</a>
										<a>第${pageno}页 / 共${pageMax}页</a>
						</div>
					</td>
				</tr>
			</table>
